...
{
res=Union(res,Box(p,vec3(1.0,1.0,1.0)));
res=Intersect(res,Sphere(p,1.3));
res=Subtract(res,CylinderX(p,vec2(0.5,2.0)));
res=Subtract(res,CylinderY(p,vec2(0.5,2.0)));
res=Subtract(res,CylinderZ(p,vec2(0.5,2.0)));
return res;
}
...
float Infinite(vec3 p,float res)
{
p=Repeat(p,vec3(4.5,4.5,4.5));
res=Union(res,MyObject(p,res));
return res;
}
